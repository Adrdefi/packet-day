...
- **`packet_back_monthly`.** Eligible: not `isPaidStatus`, not opted out, not adrdefi unless allowlisted, at least one completed packet ever, no completed packet created in the current UTC quota month. No launch cutoff. Due Pacific day 1 of the month, 1-day catch-up (through day 2) — its own, shorter window than the sequence's usual 2 days. Gated by `EMAIL_MONTHLY_ENABLED` and `EMAIL_MONTHLY_START`. `?forceMonthly=1` (still requires `CRON_SECRET`) simulates the 8am Pacific run on the 1st of **next** month (`lib/emailSequence.ts`'s `nextUtcQuotaMonth` — e.g. run it in September, it evaluates as October), bypassing `EMAIL_MONTHLY_START` and the Pacific-hour-8 gate for `packet_back_monthly` only — never `cap_followup` or the day-based sequence, and never `EMAIL_MONTHLY_ENABLED` or `?dryRun=1`, both still fully respected. It also restricts the **entire run** — every candidate source, not just monthly — to `EMAIL_TEST_ALLOWLIST` addresses, so a forced test run can never reach a real user.
