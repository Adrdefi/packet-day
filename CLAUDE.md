...
- **Switches.**
  ...
  - The mailing-address safety lock (`assertMailingAddressReady`) still runs on every real marketing send regardless of any of these.
...
- **`packet_back_monthly`.** Eligible: not `isPaidStatus`, not opted out, not adrdefi unless allowlisted, at least one completed packet ever, no completed packet created in the current UTC quota month. No launch cutoff. Due Pacific day 1 of the month, 1-day catch-up (through day 2) — its own, shorter window than the sequence's usual 2 days. Gated by `EMAIL_MONTHLY_ENABLED` and `EMAIL_MONTHLY_START`. `?forceMonthly=1` (still requires `CRON_SECRET`) treats today as the 1st and restricts the **entire run** — every candidate source, not just monthly — to `EMAIL_TEST_ALLOWLIST` addresses, so a forced test run can never reach a real user; it still respects `?dryRun=1` and the `EMAIL_MONTHLY_ENABLED` switch.
