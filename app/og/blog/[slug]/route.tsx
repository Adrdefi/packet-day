...
// Blog titles run 43-70 characters (vs. the two fixed ~60-char situation
// headlines), so the font size steps down as the title gets longer to keep
// every title within 3 lines at the 1020px content width without clipping.
function headlineFontSize(titleLength: number): number {
  if (titleLength <= 48) return 64;
  if (titleLength <= 58) return 56;
  if (titleLength <= 64) return 50;
  return 44;
}
    ...
    (
      ...
      >
        ...
        >
          ...
          <div
            style={{
              fontSize: `${headlineFontSize(post.title.length)}px`,
              fontWeight: 800,
              fontFamily: "Fraunces",
              color: "#1A1A2E",
              lineHeight: 1.15,
