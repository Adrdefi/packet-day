...
async function pageDataUri(webp: Buffer): Promise<string> {
  const jpeg = await sharp(webp).resize({ width: 700 }).jpeg({ quality: 85 }).toBuffer();
...
export async function GET() {
  ...
  const [cover, coloring] = await Promise.all([
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        {/* Right: fanned real packet pages, bleeding off the right and bottom edges */}
        <div style={{ display: "flex", flex: 1, position: "relative" }}>
          {page(coloring, 30, 178, -8)}
          {page(cover, 200, 146, 3)}
