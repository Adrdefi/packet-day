...
function GenerateContent() {
  ...
  const [upgradeModalSource, setUpgradeModalSource] = useState<
    "cap_hit" | "upgrade_link" | "generate_panel"
  >("upgrade_link");
  // Fresh server read (GET /api/plans, lib/packetUsage.ts's getPlanState,
  // same as the dashboard): true only for a free account that has already
  // used this month's packet. Swaps the form for the upgrade panel so nobody
  // fills out a form the server would refuse. null = unknown (still loading,
  // or the check failed): the form shows and the server still enforces the cap.
  const [serverCapped, setServerCapped] = useState<boolean | null>(null);
    ...
    async function load() {
      ...
      const [
        ...
        plans,
          ...
          fetch("/api/plans")
            .then((res) => (res.ok ? (res.json() as Promise<{ capped?: boolean }>) : null))
            .catch(() => null),
        ]);

      if (plans) setServerCapped(plans.capped === true);
    ...
    return (
      <div className="min-h-screen bg-cream">
        ...
        <PacketResultView
          ...
          recoveredMessage={resultWasRecovered ? GENERATION_COPY.found : undefined}
        />
      </div>
    );
  }
  ...
  // Only a confirmed server answer hides the form. atLimit (client state)
  // alone never does, per the note above.
  const showCappedPanel = !loadingData && serverCapped === true;
  ...
  return (
    <div className="min-h-screen bg-cream">
      ...
      <div className="max-w-3xl mx-auto px-4 md:px-8 py-10 space-y-10">
        {showCappedPanel ? (
          // Out of free packets this month: an upgrade panel up top instead
          // of the form, in the upgrade modal's voice.
          <div className="rounded-2xl border border-honey/30 bg-cream-dark p-6 md:p-8 text-center space-y-4">
            <h1 className="font-display text-2xl md:text-3xl font-bold text-dark leading-snug">
              {selectedChild ? `Want another packet for ${selectedChild.name}?` : "Ready for your next packet day?"}
            </h1>
            <p className="text-sm md:text-base text-dark/75 leading-relaxed max-w-md mx-auto">
              You&apos;ve used this month&apos;s free packet. Go Unlimited and turn any day into a
              packet day, for every kid in your house.
            </p>
            <div className="max-w-sm mx-auto space-y-3">
              <button
                type="button"
                onClick={() => {
                  setUpgradeModalSource("generate_panel");
                  setUpgradeModalOpen(true);
                }}
              ...
              {resetDate && (
                <p className="text-xs text-muted">
                  Your next free packet arrives {nextFreeDateLabel(resetDate)}.
                </p>
              )}
              <Link
                href="/dashboard"
                className="inline-block text-sm text-muted hover:text-dark transition-colors underline underline-offset-2"
              >
                Back to dashboard
              </Link>
            </div>
      ...
      <UpgradeModal
        ...
        capped={atLimit || serverCapped === true}
        childName={selectedChild?.name}
        childId={selectedChild?.id}
